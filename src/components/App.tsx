...
import { Pencil, Trash2, Plus, Moon, Sun, X, Check } from "lucide-react";
import { v4 as uuidv4 } from "uuid";

const STORAGE_KEY = "mes-depenses";
...
export default function App() {
  ...
  function handleSubmit(ev: React.FormEvent) {
    ...
    if (editing) {
      ...
      updated = [
        ...expenses,
        { id: uuidv4(), name: name.trim(), amount: parseFloat(amount), frequency },
      ];
    }
    setExpenses(updated);
  ...
  return (
    <div className={dark ? "dark" : ""}>
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 transition-colors">
          ...
          {expenses.length === 0 ? (
            ...
            <div className="space-y-2">
              {expenses.map((e) => (
                <div
                  key={e.id}
                  className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl px-4 py-3 flex items-center gap-3"
                >
        ...
        <button
          onClick={openAdd}
          className="fixed bottom-6 right-6 w-13 h-13 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full shadow-lg flex items-center justify-center transition-colors"
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-sm shadow-xl">
              <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100 dark:border-gray-800">
                <h2 className="font-semibold">
                  {editing ? "Modifier" : "Ajouter une dépense"}
                </h2>
                <button
                  onClick={closeForm}
              ...
              <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors"
                >
                  <Check size={16} />
